...
def train_predict_dcnn(x_prop, y_tr, x_prop_te, y_te, epochs=3, batch_size=8, lr=0.001, size=32):
...
def train_predict_efficientnetv2(x_prop, y_tr, x_prop_te, y_te, epochs=3,
                                 batch_size=4, lr=0.001, img_size=64):
...
def train_predict_stidnet(x_prop, y_tr, x_prop_te, y_te, epochs=3, batch_size=8, lr=0.001, size=32):
...
def run_model(name, x_tr, y_tr, x_te, y_te, epochs=3, **kwargs):
    if name not in MODEL_REGISTRY:
        raise KeyError(f"unknown model {name}; known={list(MODEL_REGISTRY)}")
    return MODEL_REGISTRY[name](x_tr, y_tr, x_te, y_te, epochs=epochs, **kwargs)
