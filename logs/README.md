...
| [`extract_ffpp.log`](extract_ffpp.log) | 85 | — | FaceForensics++ frame extraction / face-crop cache build (FFPP pipeline). |
| [`ffpp_merge.log`](ffpp_merge.log) | 17 | — | Merge of the 8 shard manifests into manifest.csv + summary.json, and the identity-leak assertion that gates the archive: no identity may appear in two splits. |
| [`ffpp_prepare.log`](ffpp_prepare.log) | 10 | — | FFPP prepare step: scan tree, identity grouping, shard plan. |
| [`ffpp_shard0.log`](ffpp_shard0.log) | 27 | — | FF++ face-crop extraction shard 0 of 8: every 8th video starting at index 0, via ffpp_prepare.py --shard 0 --nshards 8. |
| [`ffpp_shard1.log`](ffpp_shard1.log) | 27 | — | FF++ face-crop extraction shard 1 of 8: every 8th video starting at index 1, via ffpp_prepare.py --shard 1 --nshards 8. |
| [`ffpp_shard2.log`](ffpp_shard2.log) | 27 | — | FF++ face-crop extraction shard 2 of 8: every 8th video starting at index 2, via ffpp_prepare.py --shard 2 --nshards 8. |
| [`ffpp_shard3.log`](ffpp_shard3.log) | 27 | — | FF++ face-crop extraction shard 3 of 8: every 8th video starting at index 3, via ffpp_prepare.py --shard 3 --nshards 8. |
| [`ffpp_shard4.log`](ffpp_shard4.log) | 27 | — | FF++ face-crop extraction shard 4 of 8: every 8th video starting at index 4, via ffpp_prepare.py --shard 4 --nshards 8. |
| [`ffpp_shard5.log`](ffpp_shard5.log) | 27 | — | FF++ face-crop extraction shard 5 of 8: every 8th video starting at index 5, via ffpp_prepare.py --shard 5 --nshards 8. |
| [`ffpp_shard6.log`](ffpp_shard6.log) | 27 | — | FF++ face-crop extraction shard 6 of 8: every 8th video starting at index 6, via ffpp_prepare.py --shard 6 --nshards 8. |
| [`ffpp_shard7.log`](ffpp_shard7.log) | 27 | — | FF++ face-crop extraction shard 7 of 8: every 8th video starting at index 7, via ffpp_prepare.py --shard 7 --nshards 8. |
...
| [`kfold_true.log`](kfold_true.log) | 8964 | — | K-fold comparison, k = 6..10, stratified folds, correct scoring. Feeds section 5.6.2. The published KFAnalysis could not be used: Analysis.py:355 indexes data['image'], a key ReadDataset never stores. |
...
| [`lit_ceiling_search.log`](lit_ceiling_search.log) | 156 | — | Literature-guided representation x model search (15 reps x 8 algorithms: L1/L2 logistic, RBF/linear SVM, ExtraTrees, GBM, RF, kNN) with GridSearchCV inside training folds. Best: FAD high-band Xception + L1 logistic at 77.3% bal / 78% max acc (AUC 0.787); EfficientNetV2S frames + ExtraTrees max acc 80%. 95% NOT REACHED. Source of Optimized/lit_ceiling_search.json. ~80 min. |
...
| [`smaclmpnet_partial_20260808-043627.log`](smaclmpnet_partial_20260808-043627.log) | 73 | 299 | Snapshot of smaclmpnet_search.log taken before the run was force-stopped, preserving the fits from the uncheckpointed outer fold. |
| [`smaclmpnet_search.log`](smaclmpnet_search.log) | 74 | 299 | SMA-CLMPNet training-recipe search under nested CV (5 outer x 2 inner, 6 configurations). Force-stopped in outer fold 2; fold 1 is checkpointed in Optimized/smaclmpnet_search.json. Both published anchors scored 50.00 balanced accuracy, which on this 29/21 corpus is single-class prediction. See RESUME.md. |
